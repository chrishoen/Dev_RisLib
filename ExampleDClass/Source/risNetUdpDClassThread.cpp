...
#include "risNetUdpDClassThread.h"
...
UdpDClassThread::UdpDClassThread()
{
...
void UdpDClassThread::configure(
...
void UdpDClassThread::threadInitFunction()
{
   Prn::print(Prn::SocketInit1, "UdpDClassThread::threadInitFunction BEGIN");
   ...
   Prn::print(Prn::SocketInit1, "UdpDClassThread::threadInitFunction END");
}
...
void  UdpDClassThread::threadRunFunction()
{
   Prn::print(Prn::SocketRun1, "UdpRxDClassThread::threadRunFunction");
   ...
   while(going)
   {
      ...
      DHandle tDClass;
      if (mRxSocket.doRecvDClass(tDClass))
      {
         // Message was correctly received
         Prn::print(Prn::SocketRun1, "Recv message %d %d",mRxSocket.mRxCount,tDClass.mType);

         // Call the receive method
         processRxDClass(tDClass);
      }
      else
      {
...
void UdpDClassThread::threadExitFunction()
{
   Prn::print(Prn::SocketInit1, "UdpDClassThread::threadExitFunction");
}
...
//
// If the while loop in the threadRunFunction is blocked on doRecvDClass then
// closing the socket will cause doRecvDClass to return with false and 
// then the terminate request flag will be polled and the threadRunFunction 
// will exit.

void UdpDClassThread::shutdownThread()
{
