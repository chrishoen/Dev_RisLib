...
{
   Prn::print(0,0,"ControllerThread::threadInitFunction");
}

void ControllerThread::threadExitFunction()
{
   Prn::print(0,0,"ControllerThread::threadExitFunction");
}
...
{
   Prn::print(0,0,"ControllerThread::executeTest1 BEGIN %04d",aN);
   try
   {
      waitForTimer(aN);
   }
   ...
   Prn::print(0,0,"ControllerThread::executeTest1 END   %04d",aN);
}
...
{
   Prn::print(0,0,"ControllerThread::executeTest2 BEGIN %04d",aN);
   try
   {
      waitForNotify(aN);
   }
   ...
   Prn::print(0,0,"ControllerThread::executeTest2 END   %04d",aN);
}
...
void ControllerThread::executeSend(int aId,int aParm1,int aParm2)
{
   try
   {
      Prn::print(0,0,"ControllerThread::executeSend BEGIN     %04d",aId);

      gPlantThread->mCommandQCall(aId,aParm1,aParm2);
      BaseClass::waitForNotify();

      Prn::print(0,0,"ControllerThread::executeSend END       %04d",aId);
   }
   catch(int aStatus)
   {
...
void ControllerThread::executeSequence(int aId,int aIterations,int aCommandTimeout,int aResponseDelay)
{
   try
   {
      Prn::print(0,0,"ControllerThread::executeSequence BEGIN %04d",aId);
      ...
      case 109 :
         {
            for (int i=0;i<aIterations;i++)
            {
               Prn::print(0,0,"ControllerThread::executeSequence LOOP BEGIN %04d",i);
               gPlantThread->mCommandQCall(9,aResponseDelay,0);
               BaseClass::waitForNotifyAll(aCommandTimeout,2,9,10);
               Prn::print(0,0,"ControllerThread::executeSequence LOOP END   %04d",i);
            }
         }
         break;
      }

      Prn::print(0,0,"ControllerThread::executeSequence END   %04d",aId);
   }
   catch(int aStatus)
   {
...
{
   Prn::print(0,0,"ControllerThread::executeResponse       %04d",aId);
